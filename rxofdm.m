...
function [rxbits, conf] = rx_task1(rxsignal, conf)
    ...
    if isfield(conf.ofdm, 'scramble_seq')
        ...
        rng(42);
        scramble_seq = randi([0 1], length(conf.nbits), 1);
    end
...
function [rxbits, conf] = rx_extratask1(rxsignal, conf)
    ...
    trainPos = conf.ofdm.train_pos;
    isTrain = zeros(nSym,1);
    isTrain(trainPos) = 1;
    
    % Initialize estimation
    % --- INITIALIZATION ---
    H_est_amp = ones(N,1);                % amplitude-only channel estimate
    ...
    for k = 1:nSym
        ...
        theta_hat = mod(alpha * theta_new + (1-alpha) * theta_hat, 2*pi);
